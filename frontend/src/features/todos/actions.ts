 ...
 * KHÔNG qua giao diện của bạn. Đừng bao giờ tin rằng "chỉ form của mình mới gọi
 * được hàm này" — Server Action là một endpoint công khai như mọi endpoint khác.
 *
 * Trong dự án này, việc kiểm tra quyền xảy ra ở đâu?
 *
 *   Không phải ở đây. Mỗi hàm dưới đây gọi xuống `features/todos/api.ts`, hàm đó
 *   dùng `request()` — thứ luôn đính access token từ cookie. Rồi backend Express
 *   kiểm chữ ký token và lọc mọi query theo `userId`.
 *
 *   Nên kể cả khi kẻ tấn công gọi thẳng `deleteTodoAction(5)`, hắn vẫn chỉ xoá
 *   được todo của CHÍNH HẮN. Không có cookie hợp lệ thì không xoá được gì cả.
 *
 * Điểm mấu chốt đáng rút ra: lớp bảo vệ thật nằm ở nơi GẦN DỮ LIỆU NHẤT (câu
 * query trong `todo.service.ts`), không nằm ở tầng giao diện. Mọi thứ chạy gần
 * người dùng đều có thể bị bỏ qua.
 *
 * ---------------------------------------------------------------------------
...
import type { FormState } from "@/shared/types/api";
